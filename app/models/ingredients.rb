class Ingredient
  ...
  attr_accessor :dessert, :recipe, :ingredient

  @@all = []

  def initialize(recipe, ingredient, calories)
    @recipe = recipe
  ...
  def dessert
    Dessert.all.select {|d| d.dessert == self.recipe}
  end
  #   - #bakery
  ...
  #   - .find_all_by_name(ingredient)
    #     - should take a string argument return an array of all ingredients that include that string in their name - make sure you aren't just looking for exact matches (like 'chocolate' == 'chocolate')
