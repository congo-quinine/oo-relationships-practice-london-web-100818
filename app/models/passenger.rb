class Passenger
  ...
  def drivers
      Ride.all.map {|d| d.passenger == self
        d.driver
      }.uniq
  end

  def rides
    Ride.all.map {|r| r.passenger == self
      r.rides
    }
  end
  ...
  def self.premium_members
    prem_club = {}
    Ride.all.each { |i| prem_club[i.passenger] = 0}
    Ride.all.each { |i| prem_club[i.passenger] += i.distance }

    prem_club.select { |k,i|
      if i > 100
        k
      end
    }
